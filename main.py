...
async def send_new_student_notification(student_data):
  ...
  try:
    ...
    print(
        f"An unexpected error occurred while sending notification for {name}: {e}"
    )
...
if __name__ == '__main__':
    # This starts the Flask web server in a background thread
    keep_alive()
    print("Keep-alive server is running in the background.")

    try:
        # This runs your bot's main logic
        asyncio.run(main())
    except KeyboardInterrupt:
        print("\nExiting.")
